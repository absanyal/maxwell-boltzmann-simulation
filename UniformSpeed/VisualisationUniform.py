...
number = np.loadtxt('numberofparticlesuniform.txt', usecols=range(1))
number = int(number)
...
for n in range(1, number+1):
    Speed.append(np.loadtxt('speeduniform.txt', usecols=range(n, n+1)))
...
Timerange = np.linspace(0, 2000, 4000)
tmax= max(Timeticks)
...
Pressure.append((np.loadtxt('pressureuniform.txt', usecols=range(1,2))))
...
while not done:
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            ...
            os.remove('speeduniform.txt')
            os.remove('positionuniform.txt')
            os.remove('numberofparticlesuniform.txt')
            os.remove('pressureuniform.txt')
    if (t >= iters):
        t = 0
