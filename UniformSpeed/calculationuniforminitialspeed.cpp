...
ofstream outfile_position("positionuniform.txt");
ofstream outfile_particleno("numberofparticlesuniform.txt");
ofstream outfile_velocity("speeduniform.txt");
ofstream outfile_pressure("pressureuniform.txt");
...
/*--------- energy and momentum conservation check------------------------------------
    ...
    if(abs(check_ec) < 0.5 && abs(check_pc.mag) < 0.5)
     {
       outfile_particleno << "energy and momentum conserved \n";
     }
...
int main(){
  cout<< "------------------------------------------\n"
      << "This simulates a system of particles with all having same initial velocity but random position.\n\n"
      ...
      t_final=120;
      Dxb = 20; Dyb = 30; Dxt=800; Dyt=700;
      h=0.01;
      srand (time(NULL));

      cout << "Enter the common initial velocity components (vx,vy): ";
      cin >> ux0 >> uy0;
      cout << endl;
